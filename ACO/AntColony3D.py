...
# 城市数，蚁群
(city_num, ant_num,iter_max) = (15,60,1000)
...
if RandomNodes:
    ...
    for x in range(city_num-1):
        distance_x.append(int((random.random()*Glowidth)//1))
    distance_y=[]
    distance_y.append(10)
    for x in range(city_num-1):
        distance_y.append(int((random.random()*Gloheight)//1))
    distance_z=[]
    distance_z.append(0)
    for x in range(city_num - 1):
        distance_z.append(int((random.random()*10)//1))
else:
    distance_x = [
        10, 272, 176, 171, 650, 499, 267, 703, 408, 437,
        491, 74, 532,416, 626, 42, 271, 359, 163, 508,
        229, 576, 147, 560, 35, 714,757, 517, 64, 314,
        675, 690, 391, 628, 87, 240, 705, 699, 258,428,
        614, 36, 360, 482, 666, 597, 209, 201, 492, 294]
    distance_y = [
        10, 395, 198, 151, 242, 556, 57, 401, 305, 421,
        267, 105, 525,381, 244, 330, 395, 169, 141, 380,
        153, 442, 528, 329, 232, 48,498, 265, 343, 120,
        165, 50, 433, 63, 491, 275, 348, 222, 288,490,
        213, 524, 244, 114, 104, 552, 70, 425, 227, 331]
    distance_z = [0,1,3,2,4,6,5,0,4,3,
                  6,8,2,22,45,3,45,78,23,2,
                  23,45,6,7,3,2,1,5,6,7,8,2,
                  2,4,6,8,13,14,25,34,32,44,
                  6,12,23,14,15,23,22,4,5,16
                  ]
    # distance_z.append(0)
    # for x in range(city_num - 1):
    #     distance_z.append(0)
...
# ----------- TSP问题 -----------
class TSP(object):
    def __init__(self, root, width=Glowidth, height=Gloheight, n=city_num):
        # 创建画布
        self.root = root
        self.width = width
        self.height = height
        ...
        map_graph=[]
        for i in range(Gloheight*Glowidth):
            x=i//Glowidth
            y=i%Glowidth
            z=0
            map_graph.append((x,y,z))
            ...
            for j in xrange(city_num):
                ...
                if j<i:
                    ...
                    if (i==0 and j==city_num-1) or (j==0 and i==city_num-1):
                        ...
                        continue
                    if(abs(distance_x[i]-distance_x[j])>0.0001):
                        distance=AStarCore.getDistance(Glowidth,Gloheight,map_graph,(distance_x[i],distance_y[i],distance_z[i]),(distance_x[j],distance_y[j],distance_z[j]))
                        #temp_distance = pow(distanceall, 0.2)
                        print("单孔间距离：%d"%int(distance))
                        distance_graph[i][j] = int(distance)
                    else:
